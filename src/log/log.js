...
class DidiConsoleLogTab extends DidiConsolePlugin {
  ...
  /**
   * replace window.console with didiConsole method
   * @private
   */
  mockConsole() {
    let that = this;
  ...
  printLog(item) {
    ...
    // if didiConsole is not ready, save current log to logList
    if (!this.isReady) {
...
export default DidiConsoleLogTab;
